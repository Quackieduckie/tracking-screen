...
class OpencvFuncs():
    ...
    def __init__(self, project_path, base_ctrl):
        ...
        self.pan_angle = 0
        self.gesture_lock_active = False  # Lock face tracking after palm gesture
        self.tilt_angle = 0
        self.video_fps = 0
        self.fps_start_time = time.time()
        self.fps_count = 0
        self.cv_movtion_lock = False  # Allow servo tracking by default
        ...
        # Motion tracking cycle variables (pause-scan-move-pause)
        self.motion_scan_duration = 3.0  # Scan for motion for 3 seconds
        self.motion_pause_duration = 3.0  # Pause after moving for 3 seconds
        self.motion_cycle_start = datetime.datetime.now()
        self.motion_cycle_state = "scanning"  # States: "scanning", "moving", "paused"
        self.motion_detected_position = None
        self.motion_accumulated_detections = []
        # Face tracking cycle variables (pause-scan-move)
        self.face_scan_duration = 1.2  # Scan for faces (faster cycle)
        self.face_pause_duration = 0.8  # Pause after moving for settling (reduced)
        self.face_cycle_start = datetime.datetime.now()
        self.face_cycle_state = "scanning"  # States: "scanning", "moving", "paused"
        self.face_detected_position = None
        self.face_detected_area = 0
        ...
        if not self.usb_camera_connected:
            ...
            try:
                ...
                config = self.picam2.create_video_configuration(main={"format": 'XRGB8888', "size": (f['video']['default_res_w'], f['video']['default_res_h'])}, transform=Transform(hflip=1, vflip=1))
    ...
    def cv_detect_movition(self, img):
        ...
        if self.avg is None:
            ...
            self.motion_cycle_start = timestamp
            return
        ...
        # Calculate time in current cycle state
        time_in_state = (timestamp - self.motion_cycle_start).total_seconds()
        
        # State machine: scanning -> moving -> paused -> scanning
        if self.motion_cycle_state == "scanning":
            # SCANNING PHASE: Look for motion but don't move camera
            try:
            ...
            # Accumulate motion detections
            for c in cnts:
                if cv2.contourArea(c) < 2000:
                    continue
                (mov_x, mov_y, mov_w, mov_h) = cv2.boundingRect(c)
                area = cv2.contourArea(c)
                center = (mov_x + mov_w // 2, mov_y + mov_h // 2)
                self.motion_accumulated_detections.append({"area": area, "center": center})
                cv2.rectangle(overlay_buffer, (mov_x, mov_y), (mov_x + mov_w, mov_y + mov_h), (255, 255, 0), 1)
                self.last_movtion_captured = timestamp
            
            # Show scanning status
            cv2.putText(overlay_buffer, f'SCANNING {time_in_state:.1f}s', (10, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
            
            # After scan duration, decide if we should move
            if time_in_state >= self.motion_scan_duration:
                if len(self.motion_accumulated_detections) > 0:
                    # Find the most frequently detected area (largest accumulated area)
                    max_area = 0
                    best_center = (center_x, center_y)
                    for detection in self.motion_accumulated_detections:
                        if detection["area"] > max_area:
                            max_area = detection["area"]
                            best_center = detection["center"]
                    
                    self.motion_detected_position = best_center
                    self.motion_cycle_state = "moving"
                    self.motion_cycle_start = timestamp
                    self.motion_accumulated_detections = []
                else:
                    # No motion detected, start new scan
                    self.motion_cycle_start = timestamp
                    self.motion_accumulated_detections = []
        
        elif self.motion_cycle_state == "moving":
            # MOVING PHASE: Move to detected position
            if self.motion_detected_position and not self.cv_movtion_lock:
                cv2.circle(overlay_buffer, self.motion_detected_position, 20, (0, 255, 0), 2)
                cv2.putText(overlay_buffer, 'MOVING TO TARGET', (10, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                
                self.gimbal_track(center_x, center_y, 
                                 self.motion_detected_position[0], 
                                 self.motion_detected_position[1], 0.03)
            
            # After a short move period (0.5 seconds), enter pause
            if time_in_state >= 0.5:
                self.motion_cycle_state = "paused"
                self.motion_cycle_start = timestamp
                # Reset motion baseline to avoid detecting our own movement
                self.avg = gray.copy().astype("float")
        
        elif self.motion_cycle_state == "paused":
            # PAUSED PHASE: Let camera settle, reset baseline
            cv2.putText(overlay_buffer, f'PAUSED {time_in_state:.1f}s', (10, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (128, 128, 255), 2)
            
            # Reset motion baseline during pause
            try:
                cv2.accumulateWeighted(gray, self.avg, 0.5)
            except:
                pass
            
            # After pause duration, start new scan
            if time_in_state >= self.motion_pause_duration:
                self.motion_cycle_state = "scanning"
                self.motion_cycle_start = timestamp
                self.motion_accumulated_detections = []
                self.motion_detected_position = None
                # Reset baseline for clean scan
                self.avg = gray.copy().astype("float")
        
        # Video recording logic (keep from original)
    ...
    def gimbal_track(self, fx, fy, gx, gy, iterate):
        # Face tracking always active (gesture just sets initial position)
        global gimbal_x, gimbal_y
        # DEBUG: Log every pan movement
        with open("/tmp/pan_debug.log", "a") as dbg:
            dbg.write("TRACK: fx=" + str(int(fx)) + " gx=" + str(int(gx)) + " pan=" + str(int(self.pan_angle)) + "\n")
        # Track both horizontal (pan) and vertical (tilt) with limits
        distance = abs(fx - gx)
        
        # Calculate pan correction (inverted for 180° flipped camera)
        pan_correction = (fx - gx) * iterate
        
        # Calculate tilt correction (NEW: vertical tracking enabled)
        tilt_correction = (gy - fy) * iterate * 0.7  # Slightly slower than pan
        
        # Distance-based scaling for pan: move faster when far from center
        distance_from_center = abs(fx - gx)
        if distance_from_center > 300:  # Very far from center
            pan_correction *= 1.5  # 50% faster
        elif distance_from_center > 200:  # Moderately far
            pan_correction *= 1.3  # 30% faster
        elif distance_from_center > 100:  # Somewhat far
            pan_correction *= 1.15  # 15% faster
        
        # Cap maximum correction per frame (prevents sudden large movements)
        max_correction_per_frame = 4.0  # Maximum degrees per frame
        if abs(pan_correction) > max_correction_per_frame:
            pan_correction = max_correction_per_frame if pan_correction > 0 else -max_correction_per_frame
        
        # Cap tilt correction (gentler for vertical)
        max_tilt_correction_per_frame = 2.0  # Slower vertical movement
        if abs(tilt_correction) > max_tilt_correction_per_frame:
            tilt_correction = max_tilt_correction_per_frame if tilt_correction > 0 else -max_tilt_correction_per_frame
        ...
        if self.pan_angle > 45:
            self.pan_angle = 45
        elif self.pan_angle < -45:
            self.pan_angle = -45
        
        # TILT LIMIT: ±5 degrees only (NEW)
        if self.tilt_angle > 5:
            self.tilt_angle = 5
        elif self.tilt_angle < -5:
            self.tilt_angle = -5
    ...
    def cv_detect_faces(self, img):
        ...
        try:
            ...
            with open("/tmp/gesture.log", "a") as dbg:
                dbg.write("CHECKING HANDS\n")
            if hand_results.multi_hand_landmarks:
                with open("/tmp/gesture.log", "a") as dbg:
                    dbg.write("HAND FOUND!\n")
                for handLms in hand_results.multi_hand_landmarks:
                    h, w = img.shape[:2]
                    overlay = img.copy()  # Use copy of original image
                    # Skip mpDraw - just detect gestures
                    
                    is_palm = self.is_open_palm_simple(handLms)

                    with open("/tmp/gesture.log", "a") as dbg:
                        dbg.write("palm=" + str(is_palm) + "\n")
                    
                    if is_palm:
                        cv2.putText(overlay, "OPEN PALM -> LEFT", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                        self.pan_angle = -45
                        # Face tracking continues normally
                        self.base_ctrl.base_json_ctrl({"T": self.CMD_GIMBAL, "X": -45, "Y": self.tilt_angle, "SPD": 50, "ACC": 30})
                        self.overlay = overlay
                        return
                    elif self.is_shaka_sign(handLms):
                        with open("/tmp/pan_debug.log", "a") as dbg:
                            dbg.write("*** SHAKA/CALL DETECTED ***\n")
                        cv2.putText(overlay, "SHAKA -> RIGHT", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 128, 0), 2)
                        self.pan_angle = 45
                        self.base_ctrl.base_json_ctrl({"T": self.CMD_GIMBAL, "X": 45, "Y": self.tilt_angle, "SPD": 50, "ACC": 30})
                        self.overlay = overlay
                        return
                    else:
                        cv2.putText(overlay, "HAND (no gesture)", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
        except Exception as e:
            with open("/tmp/gesture.log", "a") as dbg:
                dbg.write("ERROR: " + str(e) + "\n")
        # === END HAND GESTURE CHECK ===
        # DEBUG: Print once every 30 frames (~1 second)
        if not hasattr(self, "_face_debug_counter"):
            self._face_debug_counter = 0
        self._face_debug_counter += 1
        if self._face_debug_counter % 30 == 0:
            print(f"[DEBUG] cv_detect_faces running, motion_lock={self.cv_movtion_lock}")
        timestamp = datetime.datetime.now()
        ...
        # Calculate time in current cycle state
        time_in_state = (timestamp - self.face_cycle_start).total_seconds()
        
        # State machine: scanning -> moving -> paused -> scanning
        if self.face_cycle_state == "scanning":
            # SCANNING PHASE: Use MediaPipe for accurate face detection
            ...
            if results.detections:
                ...
                for detection in results.detections:
                    ...
                    # MediaPipe gives confidence score (much more reliable!)
                    ...
                    # Filter border detections
                    is_not_on_border = (face_center_x > border_margin and 
                                        face_center_x < width - border_margin and
                                        face_center_y > border_margin and 
                                        face_center_y < height - border_margin)
                    
                    # Much simpler filtering - MediaPipe handles most of it
                    if is_not_on_border and face_area > 1000:  # Very permissive area check
                        # Draw with confidence score
                        cv2.rectangle(overlay_buffer,(x,y),(x+w,y+h),(64,128,255),2)
                        cv2.putText(overlay_buffer, f'{int(confidence*100)}%', (x, y-10), 
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (64,128,255), 2)
                        
                        if face_area > max_area:
                            max_area = face_area
                            max_face_center = (face_center_x, face_center_y)
                            valid_face_found = True
                    else:
                        # Draw low-confidence or border detections in red
                        cv2.rectangle(overlay_buffer,(x,y),(x+w,y+h),(0,0,255),1)
                
                # Update detected position
                if valid_face_found:
                    self.face_detected_position = max_face_center
                    self.face_detected_area = max_area
            
            # Show scanning status
            cv2.putText(overlay_buffer, f'MediaPipe SCAN {time_in_state:.1f}s', (10, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
            
            # After scan duration or if face detected outside deadzone, move
            if time_in_state >= self.face_scan_duration or (valid_face_found and time_in_state >= 0.5):
                if self.face_detected_position:
                    distance_from_center = math.sqrt((center_x - self.face_detected_position[0]) ** 2 + 
                                                     (center_y - self.face_detected_position[1]) ** 2)
                    if distance_from_center > self.aimed_error and not self.cv_movtion_lock:
                        self.face_cycle_state = "moving"
                        self.face_cycle_start = timestamp
                    else:
                        # Face is centered, restart scan
                        self.face_cycle_start = timestamp
                else:
                    # No face detected, restart scan
                    self.face_cycle_start = timestamp
        
        elif self.face_cycle_state == "moving":
            # MOVING PHASE: Move to face position, NO DETECTION
            if self.face_detected_position and not self.cv_movtion_lock:
                cv2.circle(overlay_buffer, self.face_detected_position, 20, (0, 255, 0), 2)
                cv2.putText(overlay_buffer, 'MOVING TO FACE', (10, 60), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                
                self.gimbal_track(center_x, center_y, 
                                 self.face_detected_position[0], 
                                 self.face_detected_position[1], 
                                 self.track_faces_iterate)
            
            # After movement time, pause briefly
            if time_in_state >= 0.5:
                self.face_cycle_state = "paused"
                self.face_cycle_start = timestamp
        
        elif self.face_cycle_state == "paused":
            # PAUSED PHASE: Brief settle, then resume scanning
            cv2.putText(overlay_buffer, f'PAUSED {time_in_state:.1f}s', (10, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (128, 128, 255), 2)
            
            # After pause, start new scan
            if time_in_state >= self.face_pause_duration:
                self.face_cycle_state = "scanning"
                self.face_cycle_start = timestamp
                self.face_detected_position = None
                self.face_detected_area = 0
        
        # Picture/video capture logic
        ...
        # Light control when no faces
        if self.face_cycle_state == "scanning" and self.face_detected_position is None:
            if self.cv_light_mode == 1:
                if self.base_ctrl.head_light_status != 0:
    ...
    def is_open_palm_simple(self, handLms):
        # Stricter check: ALL 4 fingers extended + thumb extended + fingers spread
        try:
            tips = [8, 12, 16, 20]  # Index, Middle, Ring, Pinky tips
            mcps = [5, 9, 13, 17]   # Corresponding MCP joints
            
            # All 4 fingers must be extended (tips above MCPs)
            extended = 0
            for tip, mcp in zip(tips, mcps):
                if handLms.landmark[tip].y < handLms.landmark[mcp].y:
                    extended += 1
            if extended < 4:  # ALL 4 fingers must be extended
                return False
            
            # Thumb must also be extended (tip far from palm center)
            thumb_tip = handLms.landmark[4]
            wrist = handLms.landmark[0]
            thumb_extended = abs(thumb_tip.x - wrist.x) > 0.12
            if not thumb_extended:
                return False
            
            # Fingers should be somewhat spread (not bunched together)
            index_tip = handLms.landmark[8]
            pinky_tip = handLms.landmark[20]
            spread = abs(index_tip.x - pinky_tip.x)
            if spread < 0.08:  # Fingers too close together
                return False
    ...
    def is_shaka_sign(self, handLms):
        # Shaka/Call gesture: Thumb + Pinky extended, Index + Middle + Ring curled
        # More lenient detection
        try:
            # Thumb should be extended (tip away from palm center)
            thumb_tip = handLms.landmark[4]
            index_mcp = handLms.landmark[5]
            wrist = handLms.landmark[0]
            # Thumb extended = thumb tip is far from index MCP (more lenient)
            thumb_extended = abs(thumb_tip.x - index_mcp.x) > 0.08
            
            # Pinky should be extended (tip above MCP, more lenient than PIP)
            pinky_tip = handLms.landmark[20]
            pinky_mcp = handLms.landmark[17]
            pinky_extended = pinky_tip.y < pinky_mcp.y
            
            # At least 2 of 3 middle fingers must be curled (tips below MCPs)
            index_curled = handLms.landmark[8].y > handLms.landmark[5].y
            middle_curled = handLms.landmark[12].y > handLms.landmark[9].y
            ring_curled = handLms.landmark[16].y > handLms.landmark[13].y
            curled_count = sum([index_curled, middle_curled, ring_curled])
            
            return thumb_extended and pinky_extended and curled_count >= 2
        except:
            return False
    
    def is_thumbs_up(self, handLms):
        # Thumbs up: thumb pointing UP, other fingers curled
        try:
            # Thumb tip should be well above thumb MCP (pointing up)
            thumb_tip = handLms.landmark[4]
            thumb_mcp = handLms.landmark[2]
            thumb_up = thumb_tip.y < thumb_mcp.y - 0.08
            
            # Other fingers should be curled
            fingers_curled = (
                handLms.landmark[8].y > handLms.landmark[6].y and  # Index
                handLms.landmark[12].y > handLms.landmark[10].y    # Middle
            )
            return thumb_up and fingers_curled
        except:
            return False
    
    def is_thumbs_down(self, handLms):
        # Thumbs down: thumb pointing DOWN, other fingers curled
        try:
            # Thumb tip should be well below thumb MCP (pointing down)
            thumb_tip = handLms.landmark[4]
            thumb_mcp = handLms.landmark[2]
            thumb_down = thumb_tip.y > thumb_mcp.y + 0.08
    ...
    def calculate_angle(self, A1, A2, B1, B2):
        try:
            ...
            if magnitude_A * magnitude_B == 0:
                return 0
            cos_val = max(-1, min(1, dot_product / (magnitude_A * magnitude_B)))
